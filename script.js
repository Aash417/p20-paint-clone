...
function createCanvas() {
	...
	contest.fillStyle = bucketColor;
	contest.fillRect(0, 0, canvas.width, canvas.height);
...
function restoreCanvas() {
	for (let i = 1; i < drawnArray.length; i++) {
		contest.beginPath();
		contest.moveTo(drawnArray[i - 1].x, drawnArray[i - 1].y);
		contest.lineWidth = drawnArray[i].size;
		contest.lineCap = "round";
		if (drawnArray[i].eraser) {
			contest.strokeStyle = bucketColor;
		} else {
			contest.strokeStyle = drawnArray[i].color;
		}
		contest.lineTo(drawnArray[i].x, drawnArray[i].y);
		contest.stroke();
	}
}
